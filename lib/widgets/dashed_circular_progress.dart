...
class DashedCircularProgressBar extends StatelessWidget {
  ...
  const DashedCircularProgressBar.square({
    ...
    this.child,
    this.valueNotifier
  }) : aspectRatio = 0, width = dimensions, height = dimensions, super(key: key);
  ...
  const DashedCircularProgressBar.aspectRatio({
    ...
    this.child,
    this.valueNotifier
  }) : width = 0, height = 0, super(key: key);
...
class _CircularProgressBarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Progress bar radius.
    double minEdge = (circleCenterAlignment == Alignment.center)? min(size.width, size.height) / 2 : min(size.width, size.height);
    double radius = minEdge - (max(foregroundStrokeWidth, backgroundStrokeWidth) / 2);
    ...
    if (seekSize > 0) {
      ...
      // Draw seek.
      canvas.drawArc(rect, startAngleRadian + sweepAngleRadian, circleSize, false, circlePaint);
    }
  }

  // This method draw multiple dash to create a dashed arc.
  void _drawDashedArc(Canvas canvas, Rect rect, double startAngleRadian, double sweepAngleRadian, Paint paint, double gapSize, double dashSize) {
  ...
  Point<double> _getCenterPoint(Size size) {
    ...
    if(circleCenterAlignment == Alignment.topRight) return Point(size.width, 0);
    // Center
    ...
    if(circleCenterAlignment == Alignment.centerRight) return Point(size.width, hh);
    // Bottom
    if(circleCenterAlignment == Alignment.bottomLeft) return Point(0, size.height);
    if(circleCenterAlignment == Alignment.bottomCenter) return Point(hw, size.height);
    if(circleCenterAlignment == Alignment.bottomRight) return Point(size.width, size.height);
    // Default
    return const Point(0, 0);
  }

  @override
  bool shouldRepaint(covariant _CircularProgressBarPainter oldDelegate) => (oldDelegate.progress != progress || oldDelegate.backgroundColor != backgroundColor || oldDelegate.foregroundColor != foregroundColor || oldDelegate.backgroundStrokeWidth != backgroundStrokeWidth || oldDelegate.foregroundStrokeWidth != foregroundStrokeWidth || oldDelegate.backgroundGapSize != backgroundGapSize || oldDelegate.backgroundDashSize != backgroundDashSize || oldDelegate.foregroundGapSize != foregroundGapSize || oldDelegate.foregroundDashSize != foregroundDashSize || oldDelegate.seekSize != seekSize || oldDelegate.seekColor != seekColor || oldDelegate.corners != corners || oldDelegate.circleCenterAlignment != circleCenterAlignment || oldDelegate.startAngle != startAngle || oldDelegate.sweepAngle != sweepAngle || oldDelegate.maxProgress != maxProgress);
